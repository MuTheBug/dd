...
def build_pdf(output_path):
    ...
    for s in [
        ...
        'أساليب التعذيب: ٣٧ أسلوب موثق + سياق ومدة وإصابات',
        'مربعات التحقق: ٢٢ مرض مزمن + ٣٧ أسلوب تعذيب + خيارات متعددة',
    ...
    story.append(Spacer(1, 6))
    story.append(Paragraph(ar('أساليب التعذيب والانتهاكات:'), styles['h2']))
    story.append(field_table(styles, [
        ('أساليب التعذيب (torture_methods)', 'مربعات تحقق', False,
         '٣٧ أسلوب - انظر القائمة أدناه'),
        ('أساليب أخرى (other_torture_methods)', 'نص', False, 'أساليب غير مذكورة'),
        ('سياق التعذيب (torture_context)', 'قائمة', False,
         '٦ خيارات - انظر أدناه'),
        ('مدة التعرض للتعذيب (torture_duration)', 'نص', False,
         'مثال: طوال فترة الاحتجاز، أول شهرين'),
        ('وصف تفصيلي للتعذيب (torture_description)', 'نص طويل', False, ''),
        ('شاهد تعذيب آخرين (torture_witnessed_others)', 'مربع تحقق', False, ''),
        ('إصابات جسدية دائمة (torture_resulted_injury)', 'مربع تحقق', False, ''),
        ('الإصابات الناتجة (torture_injuries)', 'نص', False,
         'مثال: كسور، ندوب، فقدان سمع'),
    ]))

    story.append(Spacer(1, 4))
    story.append(Paragraph(ar('قائمة أساليب التعذيب (٣٧ أسلوب):'), styles['h3']))
    story.append(opts_table(styles, [
        'الدولاب (الكاوتشوك)',
        'الصعق الكهربائي',
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(ar('خيارات سياق التعذيب (٦ خيارات):'), styles['h3']))
    story.append(code_label_table(styles, [
    ...
    story.append(info_box(
        'يتضمن النظام قسماً مخصصاً لتوثيق أساليب التعذيب والانتهاكات (٣٧ أسلوب) '
        'مع سياق التعذيب ومدته والإصابات الناتجة. انظر الخطوة ٣ للتفاصيل الكاملة.',
